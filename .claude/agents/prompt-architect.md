...
2. **Check for territory overlap before creating a new agent.** Read every existing `.claude/agents/*.md` `description` field first — do this by listing the actual directory contents at the time, not from a remembered/hardcoded roster, since the agent count changes as the repo grows. If a task fits an existing agent's stated scope, extend that agent instead of creating a near-duplicate — this repo's convention is that each agent's description carries an explicit "Not for X — use Y instead" clause stating its non-overlapping boundary against every agent it could plausibly be confused with. A new agent must state its boundary the same way.
