...
- **Never invent new visual language.** Use DTB's existing design tokens, the Geist/Nunito typography system, Lucide icon system, and the `#2255ee` primary/black Checkout Now button convention already established in `AGENTS.md` §9 — no per-product seasonal themes, no bespoke color schemes, no novelty imagery treatments.
...
- **Payment/trust marks**: never suggest adding a payment-method mark (PayPal, Klarna, Apple Pay, etc.) unless backend capability data actually supports it — marks must never imply a method is configured when it isn't (`AGENTS.md` §34.2).
- **Don't touch business identifiers** (`AGENTS.md` §34.6). SKU/MPN/GTIN/brand/taxonomy edits belong to `catalog-data` — you write the customer-facing copy around them, you don't change the identifiers themselves.
