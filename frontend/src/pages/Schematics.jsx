...
// ---------------------------------------------------------------------------
// Schematic diagram images are served exclusively from the WordPress Media
// Library via useSchematicMedia() (GET /wp-json/dtb/v1/schematics/media).
// No diagram images are bundled with the frontend build. If a schematic ID
// has no manifest entry yet (not uploaded to WP, or the API is unreachable),
// schImg()/schPrev() resolve to the shared inline placeholder below instead
// of a per-item static asset — see PLACEHOLDER_IMAGE.
// ---------------------------------------------------------------------------
...
export default function Parts() {
  ...
  // WP Media Library schematic manifest — the sole source of diagram images.
  const { manifest: schematicManifest } = useSchematicMedia();

  // Helper: resolve a diagram page URL from the WP manifest. Falls back to the
  // shared inline placeholder if this schematic/page isn't in WP yet.
  const schImg = useCallback((id, page) => {
    return schematicManifest?.[id]?.pages?.[String(page)]?.url ?? PLACEHOLDER_IMAGE;
  }, [schematicManifest]);

  // Helper: resolve a preview image URL from the WP manifest, same fallback pattern.
  const schPrev = useCallback((id) => {
    return schematicManifest?.[id]?.preview ?? PLACEHOLDER_IMAGE;
  }, [schematicManifest]);
