...
function dtb_register_cli_schematic_reconcile(): void {
	...
	WP_CLI::add_command( 'dtb schematics regenerate-oversized', 'dtb_schematic_regenerate_oversized_cli_command' );
}

/**
 * Repairs schematic page attachments that were previously downscaled by
 * WordPress's default `big_image_size_threshold` (2560px) before that
 * behavior was disabled for schematic uploads (see
 * dtb_schematic_generate_attachment_metadata_full_resolution() in
 * ReconcileSchematicSource.php). New uploads processed through
 * `wp dtb schematics reconcile --commit` are unaffected and never need this
 * command; it exists solely to repair attachments created before that fix.
 *
 * Usage:
 *   wp dtb schematics regenerate-oversized              # dry run, reports which attachments would change
 *   wp dtb schematics regenerate-oversized --commit      # writes regenerated metadata
 */
function dtb_schematic_regenerate_oversized_cli_command( array $args, array $assoc_args ): void {
	$dry_run = ! isset( $assoc_args['commit'] );

	WP_CLI::log( 'Checking schematic page attachments for a recoverable original image...' );

	$report = dtb_schematic_regenerate_oversized_run( $dry_run );

	foreach ( $report['errors'] as $error ) {
		WP_CLI::warning( $error );
	}

	if ( null !== $report['fatal_error'] ) {
		WP_CLI::error( $report['fatal_error'] );
		return;
	}

	if ( $dry_run ) {
		WP_CLI::success( sprintf( 'Dry run complete. %d of %d attachment(s) were previously scaled and would be regenerated by --commit.', $report['candidates'], $report['examined'] ) );
		return;
	}

	WP_CLI::success( sprintf( 'Complete. %d previously-scaled attachment(s) found, %d regenerated, %d failed.', $report['candidates'], $report['regenerated'], $report['failed'] ) );
}
