...
function dtb_schematics_workspace_notice_and_run(): void {
	...
	echo '<section class="notice ' . ( $ok ? 'notice-info' : 'notice-error' ) . '" aria-label="' . esc_attr__( 'Operation result', 'drywall-toolbox' ) . '"><p><strong>' . esc_html__( 'Operation run', 'drywall-toolbox' ) . ':</strong> <code>' . esc_html( $run['id'] ) . '</code> · ' . esc_html( $run['kind'] ) . ' · ' . esc_html( ! empty( $run['dry_run'] ) ? __( 'preview', 'drywall-toolbox' ) : __( 'commit', 'drywall-toolbox' ) ) . ' · ' . esc_html( $run['status'] ) . '</p>';
...
function dtb_schematics_workspace_problem_summary( DTB_Schematic_Record_Entity $record, array $health ): string {
	...
	if ( ! $record->lifecycle->is_published() && $health['requirements'] ) { $items[] = __( 'not publication-ready', 'drywall-toolbox' ); }
...
function dtb_schematics_workspace_render_record(): void {
	...
	echo '<section class="dtb-schematics-workspace__panel"><h3>' . esc_html__( 'Selected-record operations', 'drywall-toolbox' ) . '</h3><p>' . esc_html__( 'No path fields are accepted. Each operation is restricted to this record.', 'drywall-toolbox' ) . '</p>'; dtb_schematics_workspace_action_form( 'migrate_hotspots_preview', __( 'Preview hotspot migration', 'drywall-toolbox' ), $id ); dtb_schematics_workspace_action_form( 'migrate_hotspots_commit', __( 'Commit hotspot migration', 'drywall-toolbox' ), $id, true ); dtb_schematics_workspace_action_form( 'refresh_products_preview', __( 'Preview product projection', 'drywall-toolbox' ), $id ); dtb_schematics_workspace_action_form( 'refresh_products_commit', __( 'Commit product projection', 'drywall-toolbox' ), $id, true ); echo '</section>';
}
...
function dtb_schematics_workspace_render_operations(): void {
	...
	echo '<section class="dtb-schematics-workspace__panel"><h2>' . esc_html__( 'Global reconciliation', 'drywall-toolbox' ) . '</h2><p>' . esc_html__( 'The run service reads uploads/2026/schematics first and processes a bounded batch.', 'drywall-toolbox' ) . '</p>'; if ( $source['available'] ) { dtb_schematics_workspace_action_form( 'reconcile_preview', __( 'Run reconciliation preview', 'drywall-toolbox' ) ); dtb_schematics_workspace_action_form( 'reconcile_commit', __( 'Commit reconciliation batch', 'drywall-toolbox' ), 0, true ); } else { echo '<div class="notice notice-error inline"><p>' . esc_html__( 'Reconciliation is unavailable because no usable schematic source images were detected.', 'drywall-toolbox' ) . '</p></div>'; } echo '</section><section class="dtb-schematics-workspace__panel"><h2>' . esc_html__( 'Operation history', 'drywall-toolbox' ) . '</h2>'; dtb_schematics_workspace_activity_table( $history['items'] ); echo '</section>'; dtb_schematics_workspace_pagination( $page, $history['pages'], [ 'view' => 'operations' ] );
}

function dtb_schematics_workspace_activity_table( array $items ): void { echo '<table class="widefat striped"><thead><tr><th>' . esc_html__( 'When', 'drywall-toolbox' ) . '</th><th>' . esc_html__( 'Operation', 'drywall-toolbox' ) . '</th><th>' . esc_html__( 'Result', 'drywall-toolbox' ) . '</th><th>' . esc_html__( 'Summary', 'drywall-toolbox' ) . '</th></tr></thead><tbody>'; foreach ( $items as $item ) { echo '<tr><td>' . esc_html( $item['completed_at'] ) . '</td><td>' . esc_html( $item['operation_type'] ) . '</td><td>' . esc_html( $item['result'] ) . '</td><td>' . esc_html( $item['summary'] ) . '</td></tr>'; } if ( ! $items ) { echo '<tr><td colspan="4">' . esc_html__( 'No recorded operations.', 'drywall-toolbox' ) . '</td></tr>'; } echo '</tbody></table>'; }
...
function dtb_schematics_workspace_handle_action(): void {
	...
	if ( $requires_confirmation && '1' !== (string) ( $_POST['commit_confirmation'] ?? '' ) ) { dtb_schematics_workspace_redirect( $id, __( 'Confirm the commit operation before submitting it.', 'drywall-toolbox' ), 'error' ); }
