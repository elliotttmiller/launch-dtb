 ...
 * projection. Missing/invalid warehouse data is reported and skipped; it is
...
function dtb_veeqo_inventory_reconcile_page( int $page = 1, int $per_page = DTB_VEEQO_INVENTORY_PAGE_SIZE, bool $dry_run = false ) {
	...
	$result = dtb_veeqo_request(
		'GET',
		'/products',
		[ 'page' => (string) max( 1, $page ), 'page_size' => (string) min( DTB_VEEQO_INVENTORY_PAGE_SIZE, max( 1, $per_page ) ) ]
	);
...
function dtb_veeqo_inventory_merge_report( array $aggregate, array $report ): array {
	...
	foreach ( [ 'unmapped_skus', 'missing_warehouse_entries', 'invalid_stock_entries' ] as $key ) {
		$aggregate[ $key ] = array_values( array_unique( array_merge( (array) $aggregate[ $key ], (array) ( $report[ $key ] ?? [] ) ) ) );
	}
...
/** Compatibility wrapper for scheduled/system callers. */
