...
final class DTB_Veeqo_Admin_Controller {
	...
	public static function reconcile(WP_REST_Request $r): WP_REST_Response {if(!class_exists('DTB_Veeqo_Operation_Store'))return new WP_REST_Response(['ok'=>false,'code'=>'operation_store_unavailable','message'=>'Veeqo operation storage is unavailable.'],503);$result=DTB_Veeqo_Operation_Store::enqueue(rest_sanitize_boolean($r->get_param('dry_run')));return new WP_REST_Response($result,absint($result['status']??200));}
	...
	public static function retry_order(WP_REST_Request $r): WP_REST_Response {$id=absint($r['order_id']);$order=function_exists('wc_get_order')?wc_get_order($id):null;if(!$order instanceof WC_Order)return new WP_REST_Response(['success'=>false,'code'=>'order_not_found','message'=>'WooCommerce order not found.'],404);if(!function_exists('dtb_order_enqueue_job'))return new WP_REST_Response(['success'=>false,'code'=>'queue_unavailable','message'=>'The canonical DTB order queue is unavailable.'],503);$reason=substr(sanitize_text_field((string)$r->get_param('reason')),0,160);$job=dtb_order_enqueue_job('dtb_order_sync_veeqo',$id,['trigger'=>'veeqo_control_center','retry_reason'=>$reason,'operator_id'=>get_current_user_id()]);return false===$job?new WP_REST_Response(['success'=>false,'code'=>'queue_failed','message'=>'The Veeqo order retry could not be queued.'],503):new WP_REST_Response(['success'=>true,'status'=>'queued','job_id'=>$job,'message'=>sprintf('Veeqo retry queued for order #%d.',$id)],202);}
