...
function dtb_veeqo_inventory_delete_lock_value( string $current_raw ): bool {
	global $wpdb;
	$deleted = $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->options} WHERE option_name = %s AND option_value = %s", DTB_VEEQO_INVENTORY_LOCK_OPTION, $current_raw ) );
...
function dtb_veeqo_inventory_acquire_lock( int $ttl = DTB_VEEQO_INVENTORY_LOCK_TTL ): string {
	...
	$valid       = is_array( $current ) && is_string( $current['token'] ?? null ) && '' !== (string) $current['token'] && is_numeric( $current['expires_at'] ?? null );
...
function dtb_veeqo_inventory_refresh_lock( string $token, int $ttl = DTB_VEEQO_INVENTORY_LOCK_TTL ): bool {
	...
	$new_raw = wp_json_encode( [ 'token' => $token, 'expires_at' => time() + max( 60, $ttl ) ] );
	global $wpdb;
	$updated = $wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->options} SET option_value = %s WHERE option_name = %s AND option_value = %s", $new_raw, DTB_VEEQO_INVENTORY_LOCK_OPTION, $current_raw ) );
	wp_cache_delete( DTB_VEEQO_INVENTORY_LOCK_OPTION, 'options' );
	return 1 === $updated;
}
...
function dtb_veeqo_inventory_woo_ids_for_skus( array $skus ): array {
	global $wpdb;
	$skus = array_values( array_unique( array_filter( array_map( static fn( $sku ): string => trim( (string) $sku ), $skus ) ) ) );
	if ( empty( $skus ) ) {
		return [];
	}
	$lookup_table = $wpdb->prefix . 'wc_product_meta_lookup';
	$placeholders = implode( ', ', array_fill( 0, count( $skus ), '%s' ) );
	$rows = $wpdb->get_results( $wpdb->prepare( "SELECT sku, product_id FROM {$lookup_table} WHERE sku IN ({$placeholders}) ORDER BY sku ASC, product_id ASC", ...$skus ), ARRAY_A );
	$map = [];
...
function dtb_veeqo_inventory_reconcile_page( int $page = 1, int $per_page = DTB_VEEQO_INVENTORY_PAGE_SIZE, bool $dry_run = false ) {
	...
	$result = dtb_veeqo_request( 'GET', '/products', [ 'page' => (string) max( 1, $page ), 'page_size' => (string) min( DTB_VEEQO_INVENTORY_PAGE_SIZE, max( 1, $per_page ) ) ] );
	...
	$report = [ 'page' => max( 1, $page ), 'products_received' => count( $result['data'] ), 'sellables_seen' => 0, 'updated' => 0, 'unchanged' => 0, 'mapped' => 0, 'unmapped_skus' => [], 'duplicate_skus' => [], 'missing_warehouse_entries' => [], 'invalid_stock_entries' => [], 'parent_ids' => [] ];
	$page_skus = [];
	...
	foreach ( $result['data'] as $veeqo_product ) {
		...
		foreach ( (array) ( $veeqo_product['sellables'] ?? [] ) as $sellable ) {
			...
			$needs_stock = $stock['infinite'] ? ( $product->managing_stock() || ! $product->is_in_stock() ) : ( ! $product->managing_stock() || (int) $product->get_stock_quantity() !== $stock['available'] || $product->get_stock_status() !== $target_status );
			if ( $needs_meta ) {
				$report['mapped']++;
			}
...
function dtb_veeqo_inventory_reconcile_all( bool $dry_run = false, int $start_page = 1, array $aggregate = [] ) {
	...
	if ( empty( $aggregate ) ) {
		$aggregate = [ 'pages' => 0, 'sellables_seen' => 0, 'updated' => 0, 'unchanged' => 0, 'mapped' => 0, 'unmapped_skus' => [], 'duplicate_skus' => [], 'missing_warehouse_entries' => [], 'invalid_stock_entries' => [], 'dry_run' => $dry_run, 'started_at' => gmdate( 'c' ) ];
	}
	$run_started = microtime( true );
	$processed = 0;
	try {
		for ( $page = max( 1, $start_page ); $page <= 1000; $page++ ) {
			...
			if ( $processed >= DTB_VEEQO_INVENTORY_RUN_PAGES || microtime( true ) - $run_started >= DTB_VEEQO_INVENTORY_RUN_SECONDS ) {
				...
				if ( function_exists( 'as_enqueue_async_action' ) ) {
					as_enqueue_async_action( DTB_VEEQO_INVENTORY_RECONCILE_HOOK, [ 0, $page + 1, $aggregate, $dry_run ], DTB_VEEQO_INVENTORY_ACTION_GROUP, true );
				}
				return $aggregate;
			}
		}
...
function dtb_veeqo_inventory_scheduled_run( int $attempt = 0, int $start_page = 1, array $aggregate = [], bool $dry_run = false ): void {
	...
	if ( $retryable && $attempt < DTB_VEEQO_INVENTORY_MAX_RETRIES && function_exists( 'as_schedule_single_action' ) ) {
		...
		as_schedule_single_action( time() + $delay, DTB_VEEQO_INVENTORY_RECONCILE_HOOK, [ $attempt + 1, $start_page, $aggregate, $dry_run ], DTB_VEEQO_INVENTORY_ACTION_GROUP, true );
	}
	if ( function_exists( 'dtb_veeqo_log' ) ) {
		dtb_veeqo_log( 'error', 'inventory_reconciliation_failed', 'Veeqo inventory reconciliation failed.', [ 'attempt' => $attempt, 'retryable' => $retryable, 'status' => $status, 'error' => sanitize_text_field( $result->get_error_message() ) ] );
	}
}
