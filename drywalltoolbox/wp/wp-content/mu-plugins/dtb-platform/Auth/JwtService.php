...
final class DTB_JwtService {
	...
	/**
	 * Verify a DTB HS256 token and return its decoded payload.
	 *
	...
	public static function verify( string $token ) {
		...
		$expected_signature = self::base64url_encode(
			hash_hmac( 'sha256', $encoded_header . '.' . $encoded_payload, $secret, true )
		);
	...
	private static function secret() {
		...
		if ( '' === $secret ) {
			return self::error( 'auth_configuration_error', 'Authentication is unavailable.', 503 );
		}
		return $secret;
	}
	...
	private static function base64url_encode( string $value ): string {
		return rtrim(
			strtr(
				base64_encode( $value ), // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode
				'+/',
				'-_'
			),
			'='
