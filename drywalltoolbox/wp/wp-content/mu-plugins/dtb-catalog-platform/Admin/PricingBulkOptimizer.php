...
/** Register preview/apply endpoints for the whole-catalog optimizer. */
function dtb_pricing_bulk_register_routes(): void {
	$permission = static fn(): bool => current_user_can( 'dtb_manage_catalog_pricing' );

	register_rest_route(
		'dtb/v1',
		'/admin/pricing/optimize-all/preview',
		[
			'methods'             => WP_REST_Server::CREATABLE,
			'callback'            => 'dtb_pricing_bulk_rest_preview',
	...
	register_rest_route(
		'dtb/v1',
		'/admin/pricing/optimize-all/apply',
		[
			'methods'             => WP_REST_Server::CREATABLE,
			'callback'            => 'dtb_pricing_bulk_rest_apply',
			'permission_callback' => $permission,
		]
	);
}
...
/** Convert integer minor units back to the store currency amount. */
...
function dtb_pricing_bulk_rest_preview(): WP_REST_Response {
	...
	return new WP_REST_Response(
		[
			'run_token'  => $token,
			'batch_size' => DTB_PRICING_BULK_BATCH_SIZE,
			'summary'    => $summary,
		],
		200
	);
...
function dtb_pricing_bulk_rest_apply( WP_REST_Request $request ) {
	...
	$cursor = max( 0, absint( $run['cursor'] ?? 0 ) );
	$batch  = array_slice( $items, $cursor, DTB_PRICING_BULK_BATCH_SIZE );

	if ( [] !== $batch ) {
		$batch_result = dtb_pricing_apply_selected( $batch );
		$run['result']['updated']   += count( $batch_result['updated'] ?? [] );
		$run['result']['conflicts'] += count( $batch_result['conflicts'] ?? [] );
		$run['result']['errors']    += count( $batch_result['errors'] ?? [] );
		$run['cursor']               = $cursor + count( $batch );
	}

	$processed = min( count( $items ), absint( $run['cursor'] ?? 0 ) );
	...
	if ( $complete ) {
		...
		if ( function_exists( 'dtb_admin_audit_write' ) ) {
			dtb_admin_audit_write(
				'catalog_pricing',
				0,
				'catalog_pricing.optimize_all_completed',
				[
					'preview' => $run['summary'] ?? [],
					'result'  => $run['result'],
				],
		...
		set_transient( $key, $run, 30 * MINUTE_IN_SECONDS );
	}
