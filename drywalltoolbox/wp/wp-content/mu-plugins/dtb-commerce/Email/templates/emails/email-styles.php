 ...
 *
 * The base layout is intentionally fluid and email-client conservative. The
 * mobile media query refines spacing and typography only; critical shell/card
 * widths do not depend on calc(), flexbox, grid, or remote fonts.
 *
...
?>
body { background-color:<?php echo esc_attr( $palette['shell_bg'] ?? '#f2f3f5' ); ?>;font-family:<?php echo $font; ?>;margin:0;padding:0;text-align:center;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%; }
...
.dtb-email-outer-cell { padding:24px 12px; }
.dtb-email-shell-cell { background-color:#ffffff;width:100%;max-width:680px; }
#wrapper { margin:0 auto;padding:0;-webkit-text-size-adjust:100%;width:100%;max-width:680px; }
#inner_wrapper { background-color:<?php echo esc_attr( $palette['card_bg'] ?? '#ffffff' ); ?>;border:0;border-collapse:separate;width:100%; }
#template_header { background:#000000 !important;background-color:#000000 !important;border-collapse:collapse;width:100%; }
#template_header_image { background:#000000 !important;background-color:#000000 !important;padding:20px 24px; }
#template_header_image img { display:block;width:190px;max-width:100%;height:auto; }
...
#body_content { background-color:<?php echo esc_attr( $palette['card_bg'] ?? '#ffffff' ); ?>;width:100%; }
...
.email-order-details { table-layout:auto;width:100%; }
...
.address { color:#344054;font-style:normal;line-height:1.5;padding:4px 0;overflow-wrap:anywhere;word-break:break-word; }
...
.dtb-customer-detail-value { color:#101828;font-weight:600;text-align:right;overflow-wrap:anywhere;word-break:break-word; }
...
/* Shared components use percentage widths in the base markup. This is the
 * primary mobile-safety contract; the media query below is enhancement, not
 * a requirement for avoiding horizontal overflow. */
.dtb-email-hero { margin:0 0 28px !important;width:100% !important; }
...
.dtb-email-section-heading { border-bottom:1px solid #e4eaf2 !important;width:100% !important; }
#addresses { width:100% !important;table-layout:fixed; }
...
@media only screen and (max-width:600px) {
	.dtb-email-outer-cell { padding:0 !important; }
	...
	/* Gmail mobile can scale HTML background= artwork independently from the
	 * CSS background declaration. Use a deterministic solid hero on narrow
	 * clients so decorative artwork can never become an oversized foreground. */
	.dtb-email-hero { background:#030712 !important;background-color:#030712 !important;background-image:none !important;background-size:auto !important; }
	.dtb-email-hero-cell { padding:26px 20px 30px !important;background:#030712 !important;background-color:#030712 !important; }
	.dtb-email-hero h1,h1 { max-width:none !important;font-size:28px !important;line-height:1.2 !important; }
	.dtb-email-hero p { margin-bottom:10px !important;font-size:12px !important;line-height:1.4 !important; }

	#body_content_inner > p,#body_content_inner > h2 { margin-left:20px !important;margin-right:20px !important; }
	.dtb-email-card,.dtb-email-progress,.email-introduction { margin-left:auto !important;margin-right:auto !important;width:92% !important;max-width:none !important; }
	.dtb-email-card > tbody > tr > td { padding:18px 10px !important; }
	.dtb-email-section-heading { table-layout:auto !important; }
	.dtb-email-section-heading td { overflow-wrap:anywhere !important;word-break:break-word !important; }
	.dtb-progress-label span { width:auto !important;max-width:88px !important;font-size:11px !important; }

	#addresses,#addresses tbody,#addresses tr,#addresses td { display:block !important;width:100% !important;box-sizing:border-box !important;table-layout:auto !important; }
	...
	.address { overflow-wrap:anywhere !important;word-break:break-word !important; }
	...
	/* Keep the semantic three-column order table compact on mobile. Product
	 * copy is the flexible column; quantity and price remain bounded. */
	...
	.email-order-details tbody tr.order_item > td:nth-child(2) { width:32px !important;padding-left:2px !important;padding-right:2px !important;text-align:center !important;font-size:12px !important; }
	.email-order-details tbody tr.order_item > td:last-child { width:62px !important;padding-left:4px !important;padding-right:0 !important;text-align:right !important;font-size:13px !important; }
	.order-item-data { width:100% !important;table-layout:auto !important; }
	.order-item-data td { display:table-cell !important; }
	.dtb-order-item-image { width:54px !important;padding-right:7px !important; }
	.dtb-order-item-image img { width:50px !important;max-width:50px !important;height:auto !important; }
	...
	.email-order-totals tr.order-totals-shipping th,.email-order-totals tr.order-totals-shipping td { white-space:normal !important;overflow-wrap:anywhere !important;word-break:break-word !important; }
}
