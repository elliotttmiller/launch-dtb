...
Assert-NotContains $template 'checkout-flow.css' 'Custom step-flow styling must be removed'
...
Assert-NotContains $css 'dtb-mobile-checkout-actions' 'Fixed custom mobile navigation must not return'
Assert-NotContains $css 'is-dtb-checkout-step-inactive' 'Provider and native checkout sections must not be hidden'
...
Assert-NotContains $ui 'aria-hidden' 'Theme must not hide provider-owned checkout sections'
...
Assert-NotContains $ui 'innerHTML =' 'Theme must not inject duplicate form or payment markup'
