...
Assert-True ($returnContext.Contains('if ( ! self::is_confirmable_order( $order ) )')) 'Storefront success redirect must execute an explicit confirmation gate.'
...
Assert-True ($returnContext.Contains("'checkout_complete' => '1'")) 'Verified checkout completion routing must remain available after the confirmation gate.'
