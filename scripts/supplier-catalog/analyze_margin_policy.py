...
Core equations:
    ...
    map_margin_pct = (MAP - cost) / MAP * 100
    target_price = cost / (1 - target_margin_pct / 100)
    allowable_cost = price * (1 - target_margin_pct / 100)
    cost_gap = cost - allowable_cost
...
Recommended policy is descriptive decision support, not an autonomous pricing
rule:

* target margin = median eligible MAP gross margin (P50), rounded down to the
  configured policy increment so the recommendation is supported by at least
  half of the observed MAP economics;
* minimum margin = lower quartile eligible MAP gross margin (P25), rounded down
  to the policy increment, representing a conservative lower guardrail;
* no policy is recommended unless the eligible evidence count meets
  --minimum-sample-size;
* segment policies require the same minimum sample size and are emitted only as
  evidence-backed candidates for operator review.

MAP remains an independent absolute price floor in the runtime pricing engine.
"""
...
def parse_decimal(value: str | None, *, field: str, sku: str, allow_blank: bool = True, allow_zero: bool = False) -> Decimal | None:
    raw = (value or "").strip()
...
def describe(values: Iterable[Decimal]) -> dict[str, object]:
    ...
    return {
        "count": len(ordered), "min": percent(ordered[0]),
        "p10": percent(percentile(ordered, Decimal("0.10"))),
        "p25": percent(percentile(ordered, Decimal("0.25"))),
        "median_p50": percent(median), "mean": percent(mean),
        "p75": percent(percentile(ordered, Decimal("0.75"))),
        "p90": percent(percentile(ordered, Decimal("0.90"))), "max": percent(ordered[-1]),
    }


def policy_from_map_margins(values: list[Decimal], *, minimum_sample_size: int, increment: Decimal) -> dict[str, object]:
    distribution = describe(values)
    count = len(values)
    if count < minimum_sample_size:
        return {"status": "INSUFFICIENT_EVIDENCE", "eligible_count": count, "minimum_sample_size": minimum_sample_size,
                "distribution": distribution, "recommended_minimum_margin_pct": None, "recommended_target_margin_pct": None}
    ...
    return {
        "status": "EVIDENCE_AVAILABLE", "eligible_count": count, "minimum_sample_size": minimum_sample_size,
        "method": {"minimum_margin": "P25 of eligible MAP gross margins, rounded down",
        ...
        "distribution": distribution, "recommended_minimum_margin_pct": percent(minimum),
        "recommended_target_margin_pct": percent(target),
    }


def category_labels(raw: str) -> list[str]:
    return [value.strip() for value in raw.split(",") if value.strip()] or ["(Uncategorized)"]
...
def read_rows(path: Path) -> list[dict[str, str]]:
    try:
        with path.open("r", encoding="utf-8-sig", newline="") as handle:
            ...
            required = {TYPE_FIELD, SKU_FIELD, NAME_FIELD, REGULAR_FIELD, SALE_FIELD, COST_FIELD, MAP_FIELD, BRAND_FIELD, CATEGORY_FIELD}
...
def analyze_rows(rows: list[dict[str, str]], *, comparison_target_margin: Decimal, minimum_sample_size: int, policy_increment: Decimal) -> tuple[list[dict[str, str]], dict[str, object]]:
    ...
    for row in rows:
        ...
        if kind == "variable":
            ...
            continue
        if kind not in {"simple", "variation"}:
            counts["unsupported_type_excluded"] += 1
            continue
        counts["price_owning_rows"] += 1
        sku = (row.get(SKU_FIELD) or "").strip()
        name = (row.get(NAME_FIELD) or "").strip()
        brand = (row.get(BRAND_FIELD) or "").strip() or "(Unknown)"
        categories = category_labels((row.get(CATEGORY_FIELD) or "").strip())
        ...
        details.append({"sku": sku, "name": name, "type": kind, "brand": brand, "categories": " | ".join(categories),
                        "regular_price": money(regular), "sale_price": money(sale), "effective_price": money(effective),
                        "cogs": money(cost), "map_price": money(map_price), "gross_profit": money(current_profit),
                        "current_gross_margin_pct": percent(current_margin), "regular_gross_margin_pct": percent(regular_margin),
                        "current_markup_pct": percent(current_markup), "map_gross_margin_pct": percent(map_margin),
                        "comparison_target_margin_pct": percent(comparison_target_margin), "target_margin_price": money(target),
                        "allowable_cost_at_current_price": money(allowable), "cost_gap_vs_target": money(cost_gap),
                        "map_violation": "1" if map_violation else "0"})
    brand_policy = {key: policy_from_map_margins(values, minimum_sample_size=minimum_sample_size, increment=policy_increment) for key, values in sorted(by_brand.items())}
    category_policy = {key: policy_from_map_margins(values, minimum_sample_size=minimum_sample_size, increment=policy_increment) for key, values in sorted(by_category.items())}
    report: dict[str, object] = {
        "schema_version": 1, "scope": "price-owning simple products and variations; variable parents excluded",
        "counts": dict(sorted(counts.items())), "comparison_target_margin_pct": percent(comparison_target_margin),
        "current_effective_margin_distribution": describe(current_margins), "map_margin_distribution": describe(overall_map_margins),
        "recommended_global_policy": policy_from_map_margins(overall_map_margins, minimum_sample_size=minimum_sample_size, increment=policy_increment),
        "brand_policies": brand_policy, "category_policies": category_policy,
        "interpretation": {"map_margin": "Gross margin available when selling at configured official MAP.",
...
def main() -> int:
    ...
    parser.add_argument("--comparison-target-margin", default="30.00", help="Existing/proposed target margin used for target-price and cost-gap comparisons; default: 30.00")
    parser.add_argument("--minimum-sample-size", type=int, default=8, help="Minimum eligible MAP+COGS observations required before recommending a policy; default: 8")
    parser.add_argument("--policy-increment", default="0.50", help="Percentage-point increment used to round evidence-derived policy downward; default: 0.50")
    ...
    details, report = analyze_rows(rows, comparison_target_margin=comparison_target, minimum_sample_size=args.minimum_sample_size, policy_increment=policy_increment)
    ...
    print("Margin policy analysis: " f"price_owners={report['counts'].get('price_owning_rows', 0)}, " f"map+cogs={report['counts'].get('eligible_map_cost', 0)}, " f"map_violations={report['counts'].get('map_violations', 0)}, " f"policy_status={policy.get('status')}, " f"minimum={policy.get('recommended_minimum_margin_pct')}, " f"target={policy.get('recommended_target_margin_pct')}")
    return 0
