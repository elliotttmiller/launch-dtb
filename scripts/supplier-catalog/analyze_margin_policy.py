...
def percent(value: Decimal | None) -> str:
    if value is None:
        return ""
    return format(value.quantize(PERCENT_QUANTUM), "f")
...
def percentile(values: list[Decimal], probability: Decimal) -> Decimal | None:
    """Inclusive linear percentile equivalent to common descriptive Pxx usage."""
    if not values:
        return None
...
def category_labels(raw: str) -> list[str]:
    """Preserve WooCommerce category paths while supporting comma-separated memberships."""
...
def analyze_rows(
    ...
    *,
    ...
    report: dict[str, object] = {
        "schema_version": 1,
        "scope": "price-owning simple products and variations; variable parents excluded",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Minimum eligible MAP+COGS observations required before recommending a policy; default: 8",
    )
    parser.add_argument(
        "--policy-increment",
        default="0.50",
