...
Assert-Contains $provider 'wc_stripe_get_element_options' 'Stripe appearance must use the provider-supported extension point'
...
Assert-Contains $checkoutTemplate 'checkout.css' 'Native checkout must enqueue the authoritative checkout stylesheet'
Assert-Contains $checkoutTemplate 'dtb-payment-plugins-stripe-checkout' 'Checkout presentation must expose the replacement provider class'
...
Assert-NotContains $checkoutUi 'frame.style' 'Checkout UI must not modify provider iframe styles'
Assert-NotContains $checkoutCss 'confidence' 'The checkout styles must not retain a confidence panel'

Write-Host 'PASS: Product express checkout, replacement Stripe provider authority, and checkout presentation contracts are intact.'
