...
Assert-Contains $provider "'stripe_upm'" 'Universal Payment Method must be the replacement provider gateway'
...
Assert-Contains $provider "self::UPM_GATEWAY_ID !== sanitize_key" 'Standalone Payment Plugins gateways must be removed while UPM is active'
Assert-Contains $provider "unset( `$gateways['stripe'], `$gateways['woocommerce_payments'] )" 'Retired Stripe and WooPayments must be unavailable while UPM is active'
...
Assert-Contains $runtimeIntegrity "wc-stripe-upm-checkout" 'UPM checkout scripts must be protected from host optimization'
Assert-Contains $runtimeIntegrity "wc-stripe-block-upm" 'UPM Checkout Block scripts must be protected from host optimization'
Assert-Contains $capabilities "payment_plugins_stripe" 'React readiness must use the replacement provider'
Assert-Contains $capabilities "stripe_upm" 'React readiness must use the Universal Payment Method gateway'
