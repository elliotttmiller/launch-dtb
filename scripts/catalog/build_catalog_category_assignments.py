...
def build(rows: list[dict[str, str]], taxa: dict[str, dict[str, object]]) -> list[dict[str, str]]:
...
def main() -> int:
    ...
    assignments = build(rows, taxa)
    write(args.output.resolve(), assignments)
    write_brand_coverage(args.coverage.resolve(), rows, assignments)
    print(json.dumps({"catalog_rows": len(rows), "owner_assignments": len(assignments), "taxa": len(taxa)}, sort_keys=True))
    return 0
