...
- Reads the canonical catalog; never mutates it or WooCommerce.
- Uses cloudscraper with bounded retries and per-host throttling.
- Honors robots.txt by default.
- Discovers product URLs from sitemaps rather than unbounded crawling.
- Extracts JSON-LD first, then platform/product DOM fallbacks.
- Matches GTIN -> MPN/manufacturer SKU -> SKU -> conservative brand/name fuzzy match.
- Writes evidence, accepted matches, unmatched rows, and SKU-level market statistics.
- Never treats a fuzzy/ambiguous result as authoritative and never auto-updates prices.
"""
...
DEFAULT_USER_AGENT = (
    ...
    "(KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36 DTB-MarketResearch/1.0"
)
...
class HttpClient:
    ...
    def _raw_get(self, url: str, *, accept: str | None = None):
        ...
        for attempt in range(self.retries + 1):
            ...
            try:
                ...
                if response.status_code in {403, 429, 500, 502, 503, 504} and attempt < self.retries:
                    retry_after = response.headers.get("Retry-After", "")
                    try:
                        delay = min(60.0, float(retry_after)) if retry_after else min(20.0, 2.0 ** attempt)
                    except ValueError:
                        delay = min(20.0, 2.0 ** attempt)
                    logging.warning("retry status=%s url=%s delay=%.1fs", response.status_code, url, delay)
                    time.sleep(delay)
                    continue
                ...
                if attempt < self.retries:
                    delay = min(20.0, 2.0 ** attempt)
                    logging.warning("retry error=%s url=%s delay=%.1fs", exc, url, delay)
                    time.sleep(delay)
    ...
    def _robot_parser(self, url: str) -> urllib.robotparser.RobotFileParser | None:
        ...
        try:
            ...
            logging.warning("robots unavailable url=%s error=%s", robots_url, exc)
...
class MarketScraper:
    def __init__(self, *, client: HttpClient, sites: Sequence[SiteConfig], target_brands: set[str], max_urls: int, max_sitemaps: int) -> None:
        self.client = client
        self.sites = sites
    ...
    def run(self) -> tuple[list[Listing], dict[str, Any]]:
        ...
        for site in self.sites:
            site_stats = {"candidate_urls": 0, "fetched_urls": 0, "product_pages": 0, "listings": 0, "errors": 0, "robots_skips": 0}
            urls = self.discover(site)
            site_stats["candidate_urls"] = len(urls)
            logging.info("site_start key=%s candidates=%s", site.key, len(urls))
            for index, url in enumerate(urls, start=1):
                ...
                try:
                    ...
                    accepted = [item for item in parsed if self._brand_allowed(item)]
                ...
                if index % 100 == 0:
                    logging.info("site_progress key=%s processed=%s/%s listings=%s", site.key, index, len(urls), site_stats["listings"])
    ...
    def discover(self, site: SiteConfig) -> list[str]:
        queue = self._sitemap_candidates(site)
        seen_sitemaps: set[str] = set()
        urls: list[str] = []
        while queue and len(seen_sitemaps) < self.max_sitemaps and len(urls) < self.max_urls:
            ...
            queue.extend(child for child in children if child not in seen_sitemaps and sitemap_is_relevant(child))
            for location in locations:
                ...
                if same_site(candidate, site.base_url) and candidate_url(site, candidate):
                    ...
                    if len(urls) >= self.max_urls:
                        break
        return stable_unique(urls)[: self.max_urls]
...
def parse_sitemap(text: str) -> tuple[list[str], list[str]]:
    ...
    return (locations, []) if root_type == "sitemapindex" else ([], locations) if root_type == "urlset" else ([], [])
...
def listings_from_jsonld(site: SiteConfig, url: str, product: Mapping[str, Any], retrieved_at: str, source_hash: str) -> list[Listing]:
    ...
    offer_nodes = [offers] if isinstance(offers, Mapping) else [item for item in offers if isinstance(item, Mapping)] if isinstance(offers, list) else [{}]
...
def listings_from_shopify(site: SiteConfig, url: str, product: Mapping[str, Any], retrieved_at: str, source_hash: str) -> list[Listing]:
...
def listing_from_dom(site: SiteConfig, url: str, soup: BeautifulSoup, retrieved_at: str, source_hash: str) -> Listing | None:
    title = first_text(soup, ("h1.productView-title", "h1.page-title span.base", "h1.product-single__title", "h1.product__title", "h1"))
    if not title:
        return None
    sku = first_attr_or_text(soup, ("[itemprop='sku']", ".product.attribute.sku .value", ".sku", "[data-product-sku]"), ("content", "data-product-sku"))
    brand = first_attr_or_text(soup, ("[itemprop='brand']", ".productView-brand a", ".product-brand", ".brand"), ("content",))
    mpn = first_attr_or_text(soup, ("[itemprop='mpn']", ".mpn"), ("content",))
    gtin = first_attr_or_text(soup, ("[itemprop='gtin13']", "[itemprop='gtin12']", "[itemprop='gtin']", ".barcode"), ("content",))
    price = first_decimal_from_selectors(soup, (
        "[itemprop='price']", "meta[property='product:price:amount']", ".price--withoutTax",
        ".special-price .price", ".price-final_price .price", ".price-item--final", ".product__price", ".price",
    ))
    regular = first_decimal_from_selectors(soup, (".price--rrp", ".price--non-sale", ".old-price .price", ".price-item--regular", "s.price", ".was-price"))
    if price is None:
        price = first_price_from_text(soup.get_text(" ", strip=True))
    if price is None:
        return None
    availability = first_attr_or_text(soup, ("[itemprop='availability']", "link[itemprop='availability']", ".availability", ".stock"), ("href", "content")).rsplit("/", 1)[-1]
    return Listing(
        site_key=site.key,
        site_name=site.name,
        url=url,
...
def match_listings(products: Sequence[CatalogProduct], listings: Sequence[Listing], fuzzy_threshold: float) -> tuple[list[Match], list[Listing], list[CatalogProduct]]:
    ...
    matches.sort(key=lambda item: (item.dtb_brand.lower(), item.dtb_sku, item.competitor_site, item.competitor_price or Decimal("0")))
...
def write_outputs(output_dir: Path, *, products: Sequence[CatalogProduct], listings: Sequence[Listing], matches: Sequence[Match], unmatched_listings: Sequence[Listing], unmatched_products: Sequence[CatalogProduct], crawl_stats: Mapping[str, Any], args: argparse.Namespace) -> dict[str, str]:
...
def write_unmatched_catalog(path: Path, products: Sequence[CatalogProduct]) -> None:
    fields = ["sku", "name", "brand", "product_type", "current_price", "map_price", "gtin", "mpn", "manufacturer_sku", "slug"]
    with path.open("w", encoding="utf-8-sig", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields)
        writer.writeheader()
        for product in products:
...
def write_analysis(path: Path, products: Sequence[CatalogProduct], matches: Sequence[Match]) -> None:
    ...
    with path.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        for product in sorted(products, key=lambda item: (item.brand.lower(), item.sku)):
            ...
            lowest = min(rows, key=lambda row: (row.competitor_price or Decimal("999999999"), row.competitor_site), default=None)
            writer.writerow({
                "sku": product.sku,
                "name": product.name,
                "brand": product.brand,
...
def normalize_product_name(name: str, brand: str) -> str:
    ...
    return " ".join(token for token in value.split() if token not in {brand_name, "tools", "drywall", "professional"})
...
def first_decimal_from_selectors(soup: BeautifulSoup, selectors: Sequence[str]) -> Decimal | None:
    for selector in selectors:
        ...
        if element is None:
            continue
        for candidate in (element.get("content"), element.get("data-price-amount"), element.get_text(" ", strip=True)):
...
def canonicalize_url(url: str) -> str:
    ...
    query = [(key, value) for key, value in parse_qsl(parsed.query, keep_blank_values=True) if key.lower() not in TRACKING_QUERY_KEYS and not key.lower().startswith("utm_")]
...
def dedupe_listings(listings: Sequence[Listing]) -> list[Listing]:
    ...
    for listing in listings:
        key = (listing.site_key, canonicalize_url(listing.url), normalize_identifier(listing.sku or listing.mpn or listing.gtin), normalize_text(listing.variant), decimal_text(listing.current_price))
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--max-urls-per-site", type=int, default=10000)
    ...
    parser.add_argument("--ignore-robots", action="store_false", dest="respect_robots", help="Use only after verifying permission/terms for the target site")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    client = HttpClient(timeout=args.timeout, retries=args.retries, interval=args.request_interval, user_agent=args.user_agent, respect_robots=args.respect_robots)
    scraper = MarketScraper(client=client, sites=selected_sites(args.sites), target_brands=target_brands, max_urls=args.max_urls_per_site, max_sitemaps=args.max_sitemap_documents)
    ...
    paths = write_outputs(args.output_dir.resolve(), products=products, listings=listings, matches=matches, unmatched_listings=unmatched_listings, unmatched_products=unmatched_products, crawl_stats=crawl_stats, args=args)
    print(json.dumps({
