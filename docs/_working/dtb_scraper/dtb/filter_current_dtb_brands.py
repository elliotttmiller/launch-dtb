#!/usr/bin/env python3
"""Filter competitor rows to DTB-held brands without cross-brand identity contamination."""
...
def load_held_identity_index():
    held_brands: set[str] = set(); identifier_brands: dict[str, set[str]] = defaultdict(set)
...
def main() -> int:
    ...
    with SUMMARY_CSV.open("w", newline="", encoding="utf-8") as target:
        writer = csv.writer(target); writer.writerow(["metric", "value", "count"]); writer.writerow(["output_rows", "all", len(rows)]); writer.writerow(["rejected_rows", "all", sum(rejected.values())])
        for brand, count in sorted(by_brand.items()): writer.writerow(["matched_brand", brand, count])
        for method, count in sorted(methods.items()): writer.writerow(["match_method", method, count])
        for reason, count in sorted(rejected.items()): writer.writerow(["rejection_reason", reason, count])
    print(f"Wrote {len(rows)} manufacturer-supported DTB-brand competitor rows to {OUTPUT_CSV}"); return 0
