...
def official_brand(row: dict[str, str]) -> str:
    return (row.get("Brands") or row.get("Meta: _dtb_brand_label") or row.get("Meta: _dtb_brand") or row.get("Meta: schema_brand") or "").strip()


def official_ids(row: dict[str, str]) -> list[str]:
    values = [row.get("SKU", ""), row.get("Meta: schema_mpn", ""), row.get("Meta: _dtb_mpn", ""), row.get("Meta: _dtb_manufacturer_sku", "")]
    seen: set[str] = set(); result: list[str] = []
    for value in values:
        key = canonical_identifier(value)
        if key and key not in seen:
            seen.add(key); result.append((value or "").strip())
    return result


def parent_sku(row: dict[str, str]) -> str:
    return (row.get("Parent") or row.get("Meta: _dtb_parent_product_sku") or row.get("Meta: _dtb_parent_sku") or "").strip()


def load_official() -> list[dict[str, str]]:
    with OFFICIAL_CATALOG.open(newline="", encoding="utf-8-sig") as handle:
        rows = list(csv.DictReader(handle))
    by_sku = {(row.get("SKU") or "").strip(): row for row in rows if (row.get("SKU") or "").strip()}
    for index, row in enumerate(rows, start=1):
        product_type = (row.get("Type") or "").strip().casefold(); parent = parent_sku(row); parent_row = by_sku.get(parent)
        name = (row.get("Name") or "").strip(); parent_name = ((parent_row or {}).get("Name") or "").strip(); match_name = name
        if product_type == "variation" and parent_name and parent_name.casefold() not in name.casefold():
            match_name = f"{parent_name} — {name}"
        row["_row_number"] = str(index); row["_product_type"] = product_type; row["_parent_sku"] = parent; row["_parent_name"] = parent_name
...
def candidate_indexes(competitors: list[dict[str, str]]):
    by_identifier: dict[str, list[dict[str, str]]] = defaultdict(list); by_brand: dict[str, list[dict[str, str]]] = defaultdict(list); by_brand_token: dict[tuple[str, str], list[dict[str, str]]] = defaultdict(list)
    for row in competitors:
        if row["_sku_key"]: by_identifier[row["_sku_key"]].append(row)
        if row["_brand_key"]:
            by_brand[row["_brand_key"]].append(row)
            for token in row["_tokens"]: by_brand_token[(row["_brand_key"], token)].append(row)
...
def candidates_for(official: dict[str, str], by_identifier, by_brand, by_brand_token):
    ...
    if brand:
        for token in official["_tokens"]:
            ...
            if len(bucket) <= 500:
                for row in bucket: candidates.setdefault(id(row), row)
        if len(candidates) < 10 and len(by_brand.get(brand, [])) <= 1000:
            for row in by_brand.get(brand, []): candidates.setdefault(id(row), row)
...
def match_output_row(official: dict[str, str], competitor: dict[str, str], decision) -> dict[str, str]:
    competitor_price = decimal_price(competitor.get("Product Price", "")); dtb_price = decimal_price(official.get("_effective_price", "")); delta = dtb_price - competitor_price if competitor_price is not None and dtb_price is not None else None
    exact_identity = decision.method.startswith("exact_identifier") or decision.method == "exact_identity_key"
    safe_key = identity_key(competitor["_brand_key"], competitor.get("SKU", "")) if exact_identity and competitor["_brand_key"] else ""
    return {
        "Match Status": decision.status, "Match Method": decision.method, "Evidence Quality": decision.evidence_quality,
        "Match Score": str(decision.score), "WRatio": str(decision.wratio), "Token Set Ratio": str(decision.token_set), "Simple Ratio": str(decision.simple),
        "Variation Compatible": "yes" if decision.variation_compatible else "no", "Contradictions": " | ".join(decision.contradictions), "Identity Key": safe_key,
        "DTB Row": official["_row_number"], "DTB Product Type": official.get("_product_type", ""), "DTB Parent SKU": official.get("_parent_sku", ""),
...
def _market_delta_percent(dtb_price: Decimal | None, market_price: Decimal | None) -> Decimal | None:
    if dtb_price is None or market_price is None or market_price == 0: return None
...
def aggregate_row(official: dict[str, str], observations: list[dict[str, str]]) -> dict[str, str]:
    ...
    for row in observations: by_site[row["Competitor Source Key"]].append(row)
    site_evidence = [resolve_site_evidence(key, by_site.get(key, [])) for key in SITE_KEYS]
    verified_site_prices = [item.price for item in site_evidence if item.verified and item.price is not None]
    has_site_conflict = any(item.quality.startswith("conflicting_") for item in site_evidence); market = market_price_decision(verified_site_prices, has_conflict=has_site_conflict)
    dtb_price = decimal_price(official.get("_effective_price", "")); review_count = sum(1 for row in observations if row["Match Status"] == "review")
    market_delta = dtb_price - market.market_price if dtb_price is not None and market.market_price is not None else None; market_delta_percent = _market_delta_percent(dtb_price, market.market_price)
    out = {
        "DTB Row": official["_row_number"], "DTB Product Type": official.get("_product_type", ""), "DTB Parent SKU": official.get("_parent_sku", ""), "DTB SKU": official.get("SKU", ""),
        "DTB Brand": official_brand(official), "DTB Product": official.get("Name", ""), "DTB Effective Price": official.get("_effective_price", ""), "DTB Price Basis": official.get("_price_basis", ""), "DTB Price Warnings": official.get("_price_warnings", ""),
    ...
    for evidence in site_evidence:
        ...
        out[f"{label} Product"] = evidence.product_name; out[f"{label} Price"] = money(evidence.price); out[f"{label} Duplicate Count"] = str(evidence.duplicate_count)
        out[f"{label} Observed Duplicate Prices"] = " | ".join(money(value) for value in evidence.observed_prices); out[f"{label} Evidence Quality"] = evidence.quality
    return out


def write_csv(path: Path, rows: list[dict[str, str]], fields: list[str] | None = None) -> None:
    path.parent.mkdir(parents=True, exist_ok=True); fields = fields if fields is not None else (list(rows[0].keys()) if rows else [])
    with path.open("w", newline="", encoding="utf-8-sig") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields); writer.writeheader(); writer.writerows(rows)


def main() -> int:
    catalog_rows = load_official(); official_rows = [row for row in catalog_rows if row["_pricing_target"] == "yes"]; excluded_rows = [row for row in catalog_rows if row["_pricing_target"] != "yes"]
    competitors = load_competitors(); by_identifier, by_brand, by_brand_token = candidate_indexes(competitors)
    all_matches: list[dict[str, str]] = []; matches_by_official: dict[str, list[dict[str, str]]] = defaultdict(list)
    for official in official_rows:
        for competitor in candidates_for(official, by_identifier, by_brand, by_brand_token):
            decision = safe_decision(official, competitor)
            if not decision.method: continue
            row = match_output_row(official, competitor, decision); all_matches.append(row); matches_by_official[official["_row_number"]].append(row)
    all_matches.sort(key=lambda row: (row["DTB Brand"].casefold(), row["DTB Name"].casefold(), 0 if row["Match Status"] == "auto_accept" else 1, -int(row["Match Score"]), row["Competitor Source"].casefold(), row["Competitor Product Name"].casefold()))
    write_csv(OUTPUT_MATCHES, all_matches, list(all_matches[0].keys()) if all_matches else [])
    market_rows = [aggregate_row(row, matches_by_official.get(row["_row_number"], [])) for row in official_rows]; write_csv(OUTPUT_MARKET, market_rows)
    unmatched = [row for row in market_rows if row["Verified Competitor Count"] == "0" and row["Review Candidate Count"] == "0"]
    unmatched_fields = ["DTB Row", "DTB Product Type", "DTB Parent SKU", "DTB SKU", "DTB Brand", "DTB Product", "DTB Effective Price", "DTB Price Basis", "Market Price Status", "Recommended Review Status"]
    write_csv(OUTPUT_UNMATCHED, [{field: row.get(field, "") for field in unmatched_fields} for row in unmatched], unmatched_fields)
    method_counts = Counter(row["Match Method"] for row in all_matches); match_status_counts = Counter(row["Match Status"] for row in all_matches); source_counts = Counter(row["Competitor Source"] for row in all_matches)
    ...
    for value, count in sorted(source_counts.items()): summary_rows.append(["competitor_source", value, count])
    for value, count in sorted(market_status_counts.items()): summary_rows.append(["market_price_status", value, count])
    for value, count in sorted(review_status_counts.items()): summary_rows.append(["review_status", value, count])
    with OUTPUT_SUMMARY.open("w", newline="", encoding="utf-8") as handle: csv.writer(handle).writerows(summary_rows)
