...
manufacturer scoped. Canonical manufacturer identifiers preserve meaningful
separator punctuation; destructive compaction is never an identity authority.
...
same verified manufacturer product.
"""
...
def compact_identifier(value: str) -> str:
    """Loose token normalizer for fuzzy/model text only; never identity authority."""
...
def _fraction_value(whole: str | None, numerator: str | None, denominator: str | None) -> str | None:
    try:
        whole_value = Decimal(whole or "0")
        if numerator and denominator:
            denominator_value = Decimal(denominator)
            if denominator_value == 0:
                return None
            value = whole_value + (Decimal(numerator) / denominator_value)
        elif whole:
            value = whole_value
        elif numerator:
...
def extract_variation_signature(value: str) -> VariationSignature:
    ...
    for match in RANGE_RE.finditer(text):
        try:
            ranges.add(f"{Decimal(match.group(1)).normalize()}-{Decimal(match.group(2)).normalize()}")
        except InvalidOperation:
            pass
    ...
    model_tokens = {
        ...
        if compact_identifier(token) not in {compact_identifier(measurement) for measurement in measurements}
    }
    return VariationSignature(
        measurements=frozenset(measurements), ranges=frozenset(ranges), handedness=handedness,
        pack_count=pack_count, length_class=length_class, generation=generation,
        families=frozenset(families), model_tokens=frozenset(model_tokens),
    )
...
def classify_match(
    ...
    official_id_keys = {canonical_identifier(value) for value in official_identifiers if canonical_identifier(value)}
    competitor_id_key = canonical_identifier(competitor_identifier)
    ...
    contradictions = variation_contradictions(extract_variation_signature(official_name), extract_variation_signature(competitor_name))
...
def resolve_site_evidence(source_key: str, observations: Sequence[dict[str, str]]) -> SiteEvidence:
    ...
    if not verified:
        ...
        best = max(observations, key=lambda row: int(row.get("Match Score") or 0))
    ...
    if len(names) > 1:
        ...
        if any(fuzz.WRatio(canonical_text(anchor, remove_brand=True), canonical_text(name, remove_brand=True)) < 90 for name in names[1:]):
            ...
            return evidence
    prices = tuple(price for price in (decimal_price(row.get("Competitor Price", "")) for row in verified) if price is not None)
    ...
    best = max(verified, key=lambda row: (int(row.get("Match Score") or 0), int(row.get("Simple Ratio") or 0), row.get("Competitor Product Name", "")))
    evidence.product_name = best.get("Competitor Product Name", "")
    evidence.identifier = best.get("Competitor SKU", "")
    evidence.identity_key = best.get("Identity Key", "")
    return evidence
