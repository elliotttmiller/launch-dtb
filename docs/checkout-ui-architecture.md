...
Unlike an earlier (removed) checkout presentation stack that solved the mockup's step-1 field grouping by cloning proxy `first_name`/`last_name`/`phone` inputs and two-way-syncing them into the native ones, this wizard groups steps along WooCommerce's real DOM boundaries instead: name/address/phone live in the native Shipping-address block, so they appear on the Shipping screen, not Contact. Contact is Woo's actual Contact Information block (email + guest/account choice) plus the express-payment row. This is intentionally less literal to the mockup's step-1 content than a proxy-field approach would be, in exchange for zero duplicate fields, zero two-way sync code, and zero drift risk between a clone and its native source of truth.
...
## Redesign v1 — mobile pass (2026-07-28) and v2 — in-page wizard (2026-07-29)
