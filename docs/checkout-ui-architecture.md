...
- a branded top bar added to `native-checkout.php`, using the actual site logo (`/logo-white.svg`), linked to the storefront home;
...
The branded top bar's wordmark was plain text ("Dry**Wall**") instead of the store's actual logo. It now renders `/logo-white.svg` (the same asset the React storefront uses, confirmed present at `frontend/public/logo-white.svg` and its deployed copy) as an `<img>`, linked to the storefront home via `dtb_detect_storefront_base_path()` (falls back to the site root if that helper isn't available).
