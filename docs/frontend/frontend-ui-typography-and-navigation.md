...
The customer-facing label for the `products` navigation entry is **Shop** on both desktop and mobile. Its route and catalog ownership remain `/products`; only the navigation presentation label changes. Product landing CTAs such as "View all products" remain explicit and should not be mechanically renamed to "Shop."
...
Once a mega-menu renderer has been visited during the current storefront session, its renderer remains mounted but hidden when another panel is active or the sheet is closed. This preserves thumbnail DOM/image state across repeated opens instead of remounting every card after the shell exit transition. Category thumbnail URLs are additionally warmed once through `frontend/src/utils/categoryThumbnailImages.js`, which retains browser `Image` objects for the session and asynchronously decodes the resolved media. These mechanisms are presentation caches only; they do not create a second catalog or media authority, and transient image failures remain retryable.

Primary desktop nav tabs use a single pseudo-element hover interaction matching the approved scale-fill reference. The visible tab control is intentionally compact and centered inside the 98px header rather than stretching to the full header height: the standard desktop control is 44px tall with tight horizontal padding, an 8px radius, and overflow clipping. The surrounding dropdown wrapper remains full-height so pointer intent and the tab-to-sheet bridge keep their forgiving interaction area without making the visible overlay oversized. The `::before` overlay covers only this compact control surface, uses `rgba(166, 166, 166, 0.2)`, begins at `opacity: 0` and `scale(0)`, and transitions with `all 0.4s` to `opacity: 1` and `scale(1)` on hover. Keyboard `:focus-visible` receives the same animated overlay for accessibility. Active-route and open-dropdown state must not force the pseudo-element permanently expanded, because doing so bypasses the hover transition and makes the approved animation appear static. Full-header-height overlays and the former centered bordered/shadowed pill animation must not be reintroduced.
