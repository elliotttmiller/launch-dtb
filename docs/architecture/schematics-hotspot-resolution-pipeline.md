...
The canonical repository tree is `frontend/public/brands`. Frontend builds deliberately exclude that tree because the shared WordPress installation uses the independently deployed site-root `public_html/brands` tree as its schematic-hotspot source authority for both storefront environments. Staging and production storefronts consume the same normalized WordPress schematic projection rather than selecting separate source trees by request URL. A nonstandard host layout may define the server-only absolute `DTB_SCHEMATIC_HOTSPOT_SOURCE_ROOT`; request URLs never select filesystem authority. Build validation ensures every repository `schematic_data*.json` source is valid JSON and that `brands/` is absent from storefront artifacts; the owning asset deployment must separately preserve every source at the same case-sensitive path and with identical bytes under `public_html/brands`.
