...
(function ($) {
    ...
    // ---- Variation image gallery dialog ------------------------------------
    // Full ordered-thumbnail gallery editor for a single variation's images:
    // add via wp.media (multi-select when the site's variation gallery add-on
    // is enabled), drag-to-reorder, per-thumbnail remove, click-to-preview,
    // and a primary badge on index 0. Edits happen on a working copy so
    // Cancel discards them; OK commits back into state.variations[idx].images,
    // which is what the save payload serializes into image_ids (see
    // saveProduct()'s `image_ids: (v.images || []).map(...)`).
    var $varGalDlg = null;
    var varGalState = { idx: -1, images: [] };
    var varGalMediaFrame = null;

    function buildVarGalDialog() {
        if ($varGalDlg) return $varGalDlg;
        var t = PE.i18n;
        var html = '' +
            '<div class="brikpanel-pe-linkdlg brikpanel-pe-imgdlg brikpanel-pe-vargal-dlg" hidden>' +
                '<div class="brikpanel-pe-linkdlg-backdrop"></div>' +
                '<div class="brikpanel-pe-linkdlg-box brikpanel-pe-vargal-box">' +
                    '<h3 class="brikpanel-pe-linkdlg-title">' + esc(t.variation_images_title || 'Variation images') + '</h3>' +
                    '<div class="brikpanel-pe-vargal-preview" hidden><img alt=""><button type="button" class="brikpanel-pe-vargal-preview-close" aria-label="' + esc(t.close || 'Close') + '">&times;</button></div>' +
                    '<div class="brikpanel-pe-vargal-grid"></div>' +
                    '<p class="brikpanel-pe-vargal-empty brikpanel-pe-text-muted" hidden>' + esc(t.no_variation_images || 'No images yet. Add images to get started.') + '</p>' +
                    '<div class="brikpanel-pe-linkdlg-actions brikpanel-pe-vargal-actions">' +
                        '<button type="button" class="brikpanel-pe-btn secondary small brikpanel-pe-vargal-add">' + esc(t.add_images || 'Add images') + '</button>' +
                        '<span class="brikpanel-pe-vargal-spacer"></span>' +
                        '<button type="button" class="brikpanel-pe-btn secondary small brikpanel-pe-vargal-cancel">' + esc(t.cancel || 'Cancel') + '</button>' +
                        '<button type="button" class="brikpanel-pe-btn primary small brikpanel-pe-vargal-ok">' + esc(t.done || 'Done') + '</button>' +
                    '</div>' +
                '</div>' +
            '</div>';
        $varGalDlg = $(html).appendTo('body');

        function close() { $varGalDlg.attr('hidden', true); hidePreview(); }

        function hidePreview() { $varGalDlg.find('.brikpanel-pe-vargal-preview').attr('hidden', true).find('img').attr('src', ''); }
        function showPreview(url) { $varGalDlg.find('.brikpanel-pe-vargal-preview').removeAttr('hidden').find('img').attr('src', url); }

        function renderGrid() {
            var $grid = $varGalDlg.find('.brikpanel-pe-vargal-grid').empty();
            var $empty = $varGalDlg.find('.brikpanel-pe-vargal-empty');
            var imgs = varGalState.images;
            $empty.prop('hidden', imgs.length > 0);
            imgs.forEach(function (img, i) {
                var $item = $('<div class="brikpanel-pe-vargal-item" data-id="' + img.id + '">');
                if (i === 0) $item.append('<span class="brikpanel-pe-vargal-badge">' + esc(t.primary || 'Primary') + '</span>');
                $item.append('<img src="' + esc(img.url) + '" alt="">');
                var $rm = $('<button type="button" class="brikpanel-pe-vargal-remove" aria-label="' + esc(t.remove_image || 'Remove image') + '" title="' + esc(t.remove_image || 'Remove image') + '">&times;</button>');
                $item.append($rm);
                $grid.append($item);
            });
        ...
        function syncOrderFromDom() {
            var o = [];
            $varGalDlg.find('.brikpanel-pe-vargal-grid .brikpanel-pe-vargal-item').each(function () {
                var id = parseInt($(this).data('id'), 10);
                var f = varGalState.images.find(function (i) { return i.id === id; });
                if (f) o.push(f);
            });
            varGalState.images = o;
            renderGrid();
        }

        $varGalDlg.find('.brikpanel-pe-vargal-grid').sortable({
            items: '.brikpanel-pe-vargal-item', tolerance: 'pointer', cursor: 'grabbing',
            placeholder: 'brikpanel-pe-vargal-item ui-sortable-placeholder',
            update: syncOrderFromDom
        });

        $varGalDlg.data('render', renderGrid);

        $varGalDlg.find('.brikpanel-pe-vargal-cancel, .brikpanel-pe-linkdlg-backdrop').on('click', close);
        $varGalDlg.on('keydown', function (e) { if (e.key === 'Escape') close(); });

        // Click a thumbnail to preview it large; click its remove button to
        // drop it from the working copy instead (stopPropagation keeps the
        // remove click from also triggering the preview).
        $varGalDlg.find('.brikpanel-pe-vargal-grid').on('click', '.brikpanel-pe-vargal-remove', function (e) {
            e.stopPropagation();
            var id = parseInt($(this).closest('.brikpanel-pe-vargal-item').data('id'), 10);
            varGalState.images = varGalState.images.filter(function (i) { return i.id !== id; });
            renderGrid();
        });
        $varGalDlg.find('.brikpanel-pe-vargal-grid').on('click', '.brikpanel-pe-vargal-item img', function () {
            showPreview($(this).attr('src'));
        });
        $varGalDlg.find('.brikpanel-pe-vargal-preview-close').on('click', hidePreview);

        $varGalDlg.find('.brikpanel-pe-vargal-add').on('click', function () {
            // wp_localize_script flattens booleans to strings (true → "1",
            // false → ""), so check truthiness against the "1"/"0" payload.
            var galleryEnabled = String(PE.variation_gallery_enabled) === '1';
            if (!varGalMediaFrame || varGalMediaFrame._galleryEnabled !== galleryEnabled) {
                varGalMediaFrame = wp.media({
                    title: galleryEnabled ? (t.select_images || 'Select images') : (t.select_image || 'Select image'),
                    multiple: galleryEnabled,
                    library: { type: 'image' },
                    button: { text: t.select || 'Select' }
                });
                varGalMediaFrame._galleryEnabled = galleryEnabled;
                varGalMediaFrame.on('open', enableClickToToggle);
                varGalMediaFrame.on('close', disableClickToToggle);
                varGalMediaFrame.on('select', function () {
                    var attachments = varGalMediaFrame.state().get('selection').toJSON();
                    ...
                    if (galleryEnabled) {
                        // Append, de-duplicating by attachment id, preserving
                        // existing order and appending genuinely new picks.
                        newImages.forEach(function (img) {
                            if (!varGalState.images.some(function (i) { return i.id === img.id; })) {
                                varGalState.images.push(img);
                            }
                        });
                    } else {
                        // Single-image mode: the newest selection replaces
                        // whatever was there.
                        varGalState.images = newImages.slice(0, 1);
                    }
                    renderGrid();
                });
            }
            varGalMediaFrame.open();
        });

        $varGalDlg.find('.brikpanel-pe-vargal-ok').on('click', function () {
            var idx = varGalState.idx;
            var newImages = varGalState.images;
    ...
    function openVarImagePicker(idx) {
        var $dlg = buildVarGalDialog();
        varGalState.idx = idx;
        // Work on a copy so Cancel (or the backdrop/Escape) discards edits.
        varGalState.images = (state.variations[idx].images || []).slice();
        $dlg.data('render')();
        $dlg.removeAttr('hidden');
    }
